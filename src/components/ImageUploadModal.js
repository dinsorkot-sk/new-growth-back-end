...
const ImageUploadModal = ({
    ...
    const handleUploadClick = () => {
        if (files.length > 0) {
            onUpload(files, Array(files.length).fill(description));
            setFiles([]);
            setDescription('');
            onClose();
        }
        ...
        <div className="fixed inset-0 bg-[#00000050] flex items-center justify-center z-50 animate-fadeIn">
            <div className="bg-white p-6 rounded-lg max-w-md w-full transform transition-all duration-300 animate-scaleIn">
                <div className="flex justify-between items-center mb-4">
                    <h3 className="text-xl font-semibold text-gray-800">เพิ่มรูปภาพใหม่</h3>
                    <button
                        onClick={onClose}
                ...
                <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center mb-4">
                    ...
                    <label htmlFor="image-upload-input" className="cursor-pointer block">
                        <div className="flex flex-col items-center">
                            ...
                            <p className="mt-2 text-gray-600">คลิกเพื่ออัพโหลดรูปภาพ</p>
